...
class WebSocketManager(WebSocketManagerInterface):
    ...
    async def switch_to_single_symbol(self, symbol: str, category: str = "linear"):
        ...
        try:
            ...
            # CORRECTIF : Recréer un nouveau ThreadPoolExecutor après l'arrêt
            # car l'ancien a été fermé définitivement par stop()
            self._executor = concurrent.futures.ThreadPoolExecutor(
                max_workers=2,
                thread_name_prefix="ws_executor"
            )
            ...
            await self.start_connections([symbol] if category == "linear" else [], 
                                       [symbol] if category == "inverse" else [])
