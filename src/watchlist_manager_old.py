...
class WatchlistManager:
    ...
    def build_watchlist(
        ...
        funding_map = {}
        if categorie == "linear":
            # Récupération des funding rates pour linear
            funding_map = self.fetch_funding_map(base_url, "linear", 10)
        elif categorie == "inverse":
            # Récupération des funding rates pour inverse
            funding_map = self.fetch_funding_map(base_url, "inverse", 10)
        else:  # "both"
            # Récupération des funding rates pour linear+inverse
        ...
        if final_symbols:
            try:
                # Évaluation de la volatilité 5m
    ...
    def find_candidate_symbols(self, base_url: str, perp_data: Dict) -> List[str]:
        ...
        # Candidats détectés pour surveillance
        return candidates
