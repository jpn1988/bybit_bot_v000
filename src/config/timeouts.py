...
⚠️ IMPORTANT : Tous les timeouts sont centralisés ici.
Ne pas utiliser de valeurs codées en dur dans le code.
Toujours utiliser TimeoutConfig pour tous les timeouts.
"""
...
class TimeoutConfig:
    ...
    ou depuis le fichier parallel_config.yaml.

    Priorité de configuration :
    1. Variables d'environnement (PRIORITÉ MAXIMALE)
    2. Fichier parallel_config.yaml (PRIORITÉ MOYENNE)
    ...
    def _load_config(cls) -> None:
        """Charge la configuration depuis le fichier YAML si disponible."""
        if cls._config is not None:
            return

        try:
            config_path = Path("src/parallel_config.yaml")
    ...
    @classmethod
    def _get_timeout(cls, env_var: str, yaml_path: str, default: Any) -> Any:
        """
        Récupère un timeout selon la priorité : ENV > YAML > default.

        Args:
            env_var: Nom de la variable d'environnement
            yaml_path: Chemin dans le fichier YAML (ex: "timeouts.http_request")
        ...
        if env_value:
            try:
                return int(env_value) if isinstance(default, int) else float(env_value)
            except (ValueError, TypeError):
                pass

        # PRIORITÉ 2 : Fichier YAML
        cls._load_config()
        if cls._config:
            try:
